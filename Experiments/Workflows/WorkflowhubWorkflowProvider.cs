    ...
    public async IAsyncEnumerable<Workflow> GetWorkflowsAsync(WorkflowFilter filter)
    {
        ...
        foreach (var wf in response!.RootElement.GetProperty("data").EnumerateArray())
        {
            ...
            try
            {
                ...
                var tagMatch = filter.Tags.Match(workflow.Tags);
                logger.LogTrace("Matching tags - filter: {FilterTags}  wftags: {@Tags} result: {Match}",
                    filter.Tags, workflow.Tags, tagMatch);
                if (!tagMatch)
                    continue;
            }
            catch (Exception e)
